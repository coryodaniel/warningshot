  ...
  describe 'with healing enabled' do
    describe 'with heal instructions' do
      describe 'file does not exist' do
        it 'should add failed dependencies to #failed' do
          ...
          fd = WarningShot::FileResolver.new(WarningShot::Config.new,{:source  => "file://#{that_file}",:target => this_file})
          fd.test!
        ...
        it 'should heal a file from http://' do
          fd = WarningShot::FileResolver.new( WarningShot::Config.new,{:source  => "http://www.example.com/",:target => (@@dest_path / 'internetz.html')})
          fd.test!
          fd.failed.length.should be(1)
          fd.resolve!
          fd.resolved.length.should be(1)
        ...
        it 'should not increment #resolved if the resolution fails' do
          fd = WarningShot::FileResolver.new( WarningShot::Config.new,{:source  => "http://www.example.com/DOESNT.EXIST",:target => (@@dest_path / 'doesnt_exist.html')})
          fd.test!
          fd.failed.length.should be(1)
          fd.resolve!
    ...
    describe 'without heal instructions' do
      it 'should be able to return unresolved dependencies' do
        ...
        fd = WarningShot::FileResolver.new( WarningShot::Config.new,{:target => this_file})
        fd.test!
        fd.resolve!
        fd.unresolved.length.should be(1)

        fd = WarningShot::FileResolver.new WarningShot::Config.new, this_file
        fd.test!
        fd.resolve!
        fd.unresolved.length.should be(1)
      end
    ...
    describe 'with heal instructions' do      
      describe 'file does not exist' do 
        it 'should add dependency to #failed' do
          ...
          fd = WarningShot::FileResolver.new( WarningShot::Config.new,{:target => this_file,:source => that_file})
          fd.test!
          fd.failed.length.should be(1)
        end
