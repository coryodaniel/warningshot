...
class WarningShot::FileResolver
  ...
  register :test, {:name => :file_test} do |file|
    if file_found = file.exists?
      logger.debug "[PASSED] File found: #{file.target.path}"
    else
      logger.warn "[FAILED] File not found: #{file.target.path}"
    end
  ...
  register(:resolution, { :name => :file_protocol_resolver,
    ...
    begin
      ...
      logger.debug "[RESOLVED] File restored: #{file.target.path}"
    rescue Exception => ex
      logger.error "[UNRESOLVED] File not restored: #{file.target.path} from: #{file.source.path}"
    end
    file.exists?
  end
